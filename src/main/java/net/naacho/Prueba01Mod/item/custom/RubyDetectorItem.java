...
public class RubyDetectorItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        if (!pContext.getLevel().isClientSide()){
            ...
            if(!foundBlock){
                player.sendSystemMessage(Component.literal("No se encontró nada " ));
            }
        }
    ...
    private void outputValuableCoordinates(BlockPos blockPos, Player player, Block block) {
        player.sendSystemMessage(Component.literal("Se encontró " + I18n.get(block.getDescriptionId()) + " at " + "(" + blockPos.getX() + ", " + blockPos.getY() + ", " + blockPos.getZ() + ")" ));
    }
